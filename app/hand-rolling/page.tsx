...
// Keep the same Product type pattern you use on the cigars page
type Product = {
  id: string;
  name: string;
  brand: string;
  ...
  image: string;
  category: string;
  badge?: string;       // e.g. "Value Pack"
  status?: string;
  stockStatus?: string;
};

// Normalizer (accents, case, spacing)
...
export default function HandRollingCategoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [allTobacco, openBrand, selectedPair]);

  // Stock helpers (optional)
  const stockText = (p: Product) => p.stockStatus || p.status || "";
  const stockClass = (txt: string) =>
    txt === "In Stock" ? "text-green-600" : txt === "Out of Stock" ? "text-red-600" : "text-blue-600";
