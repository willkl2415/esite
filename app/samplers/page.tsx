...
export default function SamplerDetailPage({ params }: { params: { id: string } }) {
  const product = products.find((p) => p.id === params.id);

  if (!product) {
    return (
      <div className="p-10 bg-[#ff9800] min-h-screen text-center">
        <h1 className="text-2xl font-bold">Sampler not found</h1>
        <Link href="/samplers" className="text-purple hover:underline block mt-6">
          ← Back to Samplers
        </Link>
      </div>
    );
    ...
    <div className="p-10 bg-[#ff9800] min-h-screen">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="text-center mt-8">
          <Link href="/samplers" className="text-purple hover:underline">
            ← Back to Samplers
          </Link>
        </div>
      </div>
